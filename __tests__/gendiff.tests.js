...
test('Check diff for flat JSON files with stylish output format', () => {
  ...
  test('Check diff for YAML and JSON files with JSON output format', () => {
    expect(genDiff(getFixturePath('nestfile1.yaml'), getFixturePath('nestfile2.yaml'), 'json')).toEqual(readFixtureFile('expRes7.txt'));
    expect(genDiff(getFixturePath('nestfile1.json'), getFixturePath('nestfile2.json'), 'json')).toEqual(readFixtureFile('expRes8.txt'));
  });